...
async def send_audio_and_get_transcription(file_path):
    async with websockets.connect(SERVER_URI, max_size=50 * 1024 * 1024) as ws:
        # 🔹 Abrimos el audio y leemos propiedades
        info = sf.info(file_path)

        print(f"✅ Archivo: {file_path}")
        print(f"   Frecuencia (samplerate): {info.samplerate} Hz")
        ...
        data = librosa.resample(data, orig_sr=orig_sr, target_sr=16000)  # 🔹 resample a 16kHz
        if len(data.shape) > 1:  # 🔹 convertir estéreo a mono
            data = data.mean(axis=1)
        samplerate = 16000
        # Convertimos a bytes PCM 32-bit
        pcm_data = (data * 32767).astype(np.int16).tobytes()
        
        # Tamaño del chunk de 0.1s (100ms)
        samples_per_chunk = int(samplerate * 0.1)  # 0.1s en muestras
        bytes_per_sample = 2 * (data.shape[1] if len(data.shape) > 1 else 1)  # canales
        ...
        # 🔹 Enviamos en chunks de 100ms
        start_time = time.perf_counter()
        for i in range(0, len(pcm_data), chunk_size):
            chunk = pcm_data[i:i+chunk_size]
            await ws.send(chunk)
            await asyncio.sleep(0.1)  # simula envío en tiempo real

        # 🔹 Avisamos fin
        await ws.send("__END__")
        print("🏁 Enviado END")

        # 🔹 Recibimos transcripción final
